...
function isFinishedTicket(status) {
  return ['resolved', 'archived'].includes(
...
async function ensureMessageSchema() {
  ...
  if (rows[0] && rows[0].DATA_TYPE !== 'text' && rows[0].DATA_TYPE !== 'longtext') {
    await pool.query(
      'ALTER TABLE ticket_messages MODIFY COLUMN attachment TEXT DEFAULT NULL'
    );
  }
...
async function sendMessageController(req, res) {
  ...
  try {
    ...
    if (!ticket) {
      if (req.file?.path) await removeLocalFileQuietly(req.file.path);
    ...
    if (isFinishedTicket(ticket.status)) {
      if (req.file?.path) await removeLocalFileQuietly(req.file.path);
      return res.status(400).json({
        error: 'This ticket is already resolved or archived',
      });
    }

    if (req.file) {
      uploadedAsset = await uploadLocalFileMaybe(req.file, 'cignalcare/chat');

      if (process.env.NODE_ENV !== 'production') {
        console.log('CHAT ATTACHMENT STORED:', {
          storage: uploadedAsset?.storage || 'unknown',
          mimeType: req.file.mimetype,
          sizeBytes: req.file.size,
          hasUrl: Boolean(uploadedAsset?.url),
        });
      }
    }

    const attachment = uploadedAsset?.url || null;
    ...
    try {
      if (senderRole === 'admin') {
        ...
        await createAdminNotification({
          type: 'admin_message',
          message: `New customer reply on ticket #${ticket.id} from ${
            req.user.accountName || ticket.accountName || 'Customer'
          }.`,
        });
      }
    } catch (notificationError) {
      console.error('CHAT NOTIFICATION ERROR:', notificationError.message);
    }

    const [rows] = await pool.query(
      `SELECT tm.*, u.accountName, u.accountName AS sender_name
    ...
    const savedMessage = rows[0] || {
      ...
      message: hasMessage ? messageText : null,
      attachment,
      attachment_type: attachmentType,
      accountName: req.user.accountName || null,
      sender_name: req.user.accountName || null,
    ...
    return res.status(201).json({
      ...
      attachment_storage: uploadedAsset?.storage || null,
    });
  } catch (error) {
    console.error('SEND MESSAGE ERROR', error);

    if (!messagePersisted) {
      if (uploadedAsset?.storage === 'cloudinary' && uploadedAsset.publicId) {
      ...
      if (req.file?.path) {
        await removeLocalFileQuietly(req.file.path);
      }
    }

    if (
      error.message?.includes('Only JPG') ||
      error.message?.includes('Upload must be')
    ) {
  ...
  try {
    ...
    const [rows] = await pool.query(
      `SELECT
         ...
         CASE WHEN tm.sender_id = ? THEN 1 ELSE 0 END AS is_mine
       FROM ticket_messages tm
       LEFT JOIN users u ON u.id = tm.sender_id
       WHERE tm.ticket_id = ?
