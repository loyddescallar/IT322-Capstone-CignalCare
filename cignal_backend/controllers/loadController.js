...
const ALLOWED_STATUS = ['pending', 'completed', 'cancelled', 'failed'];
const PLAN_STATUSES = ['active', 'inactive'];
const POS_PAYMENT_METHODS = new Set(['Cash', 'GCash', 'Maya', 'Bank Transfer']);
...
function decoratePlan(plan) {
  const channels = normalizeChannels(safeJsonParse(plan.channels_json, []));
...
async function ensurePlanColumns() {
  ...
  const [rows] = await pool.query(
    `SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS
     WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'prepaid_plans'`
  );

  const existing = new Set(rows.map((row) => row.COLUMN_NAME));

  if (!existing.has('channels_json')) {
    await pool.query('ALTER TABLE prepaid_plans ADD COLUMN channels_json LONGTEXT DEFAULT NULL AFTER benefits_text');
  }
...
function cleanPlanPayload(body) {
  ...
  const planCode = String(body.plan_code || body.planCode || planName.replace(/\s+/g, '').toUpperCase()).trim();
  const amount = Number(body.amount || 0);
  const validityDays = Number(body.validity_days || body.validityDays || 30);
  const hdChannels = Number(body.hd_channels ?? body.hdChannels ?? 0);
  const sdChannels = Number(body.sd_channels ?? body.sdChannels ?? 0);
  const benefitsText = String(body.benefits_text ?? body.benefitsText ?? '').trim();
  const aiNote = String(body.ai_note ?? body.aiNote ?? '').trim();
  const status = PLAN_STATUSES.includes(body.status) ? body.status : 'active';
...
function validatePlanPayload(plan) {
  if (!plan.planName) return 'Plan name is required';
  if (!plan.planCode) return 'Plan code is required';
  if (!plan.amount || Number(plan.amount) <= 0) return 'Amount must be greater than zero';
  if (!plan.validityDays || Number(plan.validityDays) <= 0) return 'Validity days must be greater than zero';
  return '';
}
...
async function addLoad(req, res) {
  try {
    ...
    if (!accountNumber || (!planId && !loadAmount)) {
      return res.status(400).json({
        error: 'accountNumber and planId are required',
      });
    }

    const cleanPaymentMethod = String(paymentMethod || '').trim();

    if (!POS_PAYMENT_METHODS.has(cleanPaymentMethod)) {
    ...
    const result = await processPosLoad({
      ...
      processed_by: req.user?.accountName || 'Admin',
      reference_no: makePosReference(),
      description: String(description || '').trim() || null,
    });

    await notifySafely('POS LOAD', () =>
      createNotification({
        user_id: customer.id,
        account_number: customer.accountNumber,
        type: 'load_completed',
        message: `Your ${result.plan.plan_name} load (₱${Number(result.plan.amount).toLocaleString()}) was processed by Descallar Satellite Services.`,
      })
    );

    return res.status(201).json({
      message: 'Load transaction recorded',
...
async function getPlansController(req, res) {
  try {
    ...
    const sql = includeInactive
      ? 'SELECT * FROM prepaid_plans ORDER BY amount ASC'
      : "SELECT * FROM prepaid_plans WHERE status = 'active' ORDER BY amount ASC";

    const [plans] = await pool.query(sql);
    return res.json({ success: true, plans: plans.map(decoratePlan) });
  } catch (err) {
...
async function createPlanController(req, res) {
  try {
    await ensurePlanColumns();
    const plan = cleanPlanPayload(req.body);
    const validationError = validatePlanPayload(plan);

    if (validationError) return res.status(400).json({ error: validationError });

    const [result] = await pool.query(
      `INSERT INTO prepaid_plans
       (plan_code, plan_name, amount, validity_days, hd_channels, sd_channels, benefits_text, channels_json, ai_note, status)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        plan.planCode,
        plan.planName,
    ...
    return res.status(201).json({ success: true, id: result.insertId, message: 'Plan created' });
  } catch (err) {
    console.error('CREATE PLAN ERROR', err);
    if (err.code === 'ER_DUP_ENTRY') return res.status(409).json({ error: 'Plan code already exists' });
    return res.status(500).json({ error: 'Server error' });
  }
}
  ...
  try {
    ...
    const [result] = await pool.query(
      `UPDATE prepaid_plans
       SET plan_code = ?, plan_name = ?, amount = ?, validity_days = ?, hd_channels = ?, sd_channels = ?,
           benefits_text = ?, channels_json = ?, ai_note = ?, status = ?
       WHERE id = ?`,
      [
        plan.planCode,
        plan.planName,
...
async function deletePlanController(req, res) {
  try {
    await ensurePlanColumns();
    const [result] = await pool.query('DELETE FROM prepaid_plans WHERE id = ?', [req.params.id]);
    ...
    if (err.code === 'ER_ROW_IS_REFERENCED_2' || err.code === 'ER_ROW_IS_REFERENCED') {
      return res.status(409).json({ error: 'This plan already has transaction records. Set it to inactive instead of deleting it.' });
    }
    return res.status(500).json({ error: 'Server error' });
  }
}
