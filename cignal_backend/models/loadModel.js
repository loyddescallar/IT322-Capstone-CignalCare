  ...
  try {
    ...
    if (Number.isInteger(parsedPlanId) && parsedPlanId > 0) {
      const [rows] = await connection.query(
        "SELECT * FROM prepaid_plans WHERE id=? AND status='active' LIMIT 1 FOR UPDATE",
        [parsedPlanId]
      );
      plan = rows[0] || null;
    }
    ...
    if (!plan) {
      const [rows] = await connection.query(
        "SELECT * FROM prepaid_plans WHERE amount=? AND status='active' ORDER BY id ASC LIMIT 1 FOR UPDATE",
        [entry.loadAmount]
      );
      plan = rows[0] || null;
    }
    ...
    if (!plan) {
      const error = new Error('Selected prepaid plan is unavailable or inactive.');
    ...
    const [history] = await connection.query(
      `INSERT INTO load_history
       ...
       VALUES (?, ?, ?, ?, 'completed')`,
      [entry.user_id, entry.accountNumber, plan.amount, description]
    );
...
module.exports={processPosLoad,addLoadHistory,getLoadHistoryByUser,getAllLoadHistory,getAllPrepaidTransactions,updateLoadStatus};
