...
export default function AdminPOS() {
  ...
  const [processing,setProcessing]=useState(false); const [success,setSuccess]=useState(null); const [error,setError]=useState(''); const [qrModal,setQrModal]=useState(false); const [copied,setCopied]=useState('');
  ...
  const handleSearch=e=>{ e.preventDefault(); const q=ccaQ.trim(); if(!q)return; const f=customers.find(c=>c.accountNumber===q||c.ccaNumber===q); setFound(f||null);setNotFound(!f);setPlan(null);setSuccess(null);setError(''); };
  const copyText=(text,key)=>{ navigator.clipboard.writeText(text).then(()=>{setCopied(key);setTimeout(()=>setCopied(''),2000);}); };
  const handleProcess=async()=>{
    if(!found||!plan||processing)return;
    if(payMethod==='Cash'&&(!cashTendered||Number(cashTendered)<Number(plan.amount))){setError('Enter a cash amount equal to or greater than the selected plan.');return;}
    setProcessing(true);setError('');setSuccess(null);
    try{await axiosClient.post('/load',{accountNumber:found.accountNumber,planId:plan.id,loadAmount:plan.amount,paymentMethod:payMethod,description:'POS Load — '+plan.plan_name+' via '+payMethod}); setSuccess({customer:found.accountName,plan:plan.plan_name,amount:plan.amount}); setFound(null);setPlan(null);setCcaQ('');setNotFound(false);setCash('');setQrModal(false); const hr=await axiosClient.get('/load/admin');setHistory(hr.data?.history||[]);}catch(e){console.error(e);setError(e.response?.data?.error||'Unable to process this load transaction.');}finally{setProcessing(false);}
  };
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5 space-y-4">
          ...
          {error&&<div className="bg-red-50 border border-red-200 rounded-xl p-3 flex items-start gap-2"><X size={14} className="text-red-600 flex-shrink-0 mt-0.5"/><p className="text-xs font-semibold text-red-700">{error}</p></div>}
          ...
          <div>
            ...
            <form onSubmit={handleSearch} className="flex flex-col gap-2 sm:flex-row"><input type="text" value={ccaQ} onChange={e=>{setCcaQ(e.target.value);setNotFound(false);setFound(null);}} placeholder="Account No. or CCA No." className="min-w-0 flex-1 rounded-xl border border-gray-200 px-3 py-2.5 text-xs outline-none focus:border-[#cc0000]"/><button type="submit" className="w-full rounded-xl bg-[#cc0000] px-4 py-2.5 text-xs font-semibold text-white hover:bg-red-700 sm:w-auto">Search</button></form>
          ...
          {found&&plan&&<div className="border-t border-gray-100 pt-3 space-y-1.5">
            ...
            <button onClick={handleProcess} disabled={processing||(payMethod==='Cash'&&(!cashTendered||Number(cashTendered)<Number(plan.amount)))} className="w-full bg-[#cc0000] hover:bg-red-700 text-white text-xs py-3 rounded-xl font-bold mt-2 disabled:opacity-60">{processing?'Processing...':'✓ Process ₱'+Number(plan.amount).toLocaleString()+' Load'}</button>
          </div>}
        </div>
        <div className="bg-white rounded-xl shadow-sm border border-gray-100">
          ...
          <div className="divide-y divide-gray-50 max-h-[460px] overflow-y-auto">{loading?<div className="py-8 text-center text-xs text-gray-400">Loading...</div>:displayedHistory.length===0?<div className="py-8 text-center text-xs text-gray-400">No transactions.</div>:displayedHistory.slice(0,25).map((h,i)=><div key={i} className="flex items-start justify-between gap-3 px-4 py-3 hover:bg-gray-50"><div><p className="text-xs font-semibold text-gray-800">{h.accountNumber}</p><p className="text-xs text-gray-400 mt-0.5">{h.description||'Prepaid Load'}</p><p className="text-gray-400 mt-0.5" style={{fontSize:'10px'}}>{new Date(h.created_at).toLocaleDateString('en-PH')}</p></div><p className="text-sm font-bold text-[#cc0000]">₱{Number(h.loadAmount||0).toLocaleString()}</p></div>)}</div>
        </div>
      </div>
      {qrModal&&plan&&(
          ...
          <div className="max-h-[calc(100dvh-1.5rem)] w-full max-w-sm overflow-y-auto rounded-2xl bg-white shadow-2xl">
            ...
            <div className="p-5 space-y-4">
              ...
              <div className={`rounded-xl p-4 space-y-3 ${payMethod==='GCash'?'bg-blue-50 border border-blue-100':'bg-emerald-50 border border-emerald-100'}`}>
                ...
                <div className="flex flex-col items-stretch justify-between gap-2 sm:flex-row sm:items-center"><div><p className="text-xs text-gray-500" style={{fontSize:'10px'}}>{payMethod} NUMBER</p><p className="text-xs font-bold text-gray-800 mt-0.5 font-mono">{payAccount.number}</p></div><button onClick={()=>copyText(payAccount.number,'number')} className={`flex items-center gap-1 text-xs px-3 py-1.5 rounded-lg font-semibold text-white ${payMethod==='GCash'?'bg-blue-600':'bg-emerald-600'}`}>{copied==='number'?<Check size={12}/>:<Copy size={12}/>}{copied==='number'?'Copied!':'Copy'}</button></div>
                <div className="flex flex-col items-stretch justify-between gap-2 border-t border-gray-200 pt-2 sm:flex-row sm:items-center"><div><p className="text-xs text-gray-500" style={{fontSize:'10px'}}>AMOUNT TO SEND</p><p className="text-lg font-bold text-[#cc0000]">₱{Number(plan.amount).toLocaleString()}</p></div><button onClick={()=>copyText(String(plan.amount),'amount')} className={`flex items-center gap-1 text-xs px-3 py-1.5 rounded-lg font-semibold text-white ${payMethod==='GCash'?'bg-blue-600':'bg-emerald-600'}`}>{copied==='amount'?<Check size={12}/>:<Copy size={12}/>}{copied==='amount'?'Copied!':'Copy'}</button></div>
              </div>
